...
const Taurus = () => {
  ...
  useEffect(() => {
    ...
    const handlePopState = () => {
      console.log('Taurus AR: Browser back button detected - cleaning up AR session');
      cleanupARSession();
  ...
  useEffect(() => {
    ...
    let imageSrc = '/models/2D/taurus.png';
    if (currentView === 'withNames') imageSrc = '/models/2D/taurus-with-name.png';
    else if (currentView === 'withSign') imageSrc = '/models/2D/taurus-with-sign.png';

    console.log('Taurus - Current view:', currentView);
    console.log('Taurus - Image source:', imageSrc);
  ...
  const getInfoContent = () => {
    if (currentView === 'withNames') {
      return { title: '✨ Taurus with Star Names', text: 'This view shows Taurus the Bull with labeled stars including Aldebaran (the red giant "eye of the bull"), Elnath (the bull\'s horn), and the famous star clusters: the Pleiades (Seven Sisters) and the Hyades (V-shaped face). Aldebaran appears to be part of the Hyades but is actually much closer to Earth!' };
    } else if (currentView === 'withSign') {
      return { title: '♉ Taurus the Bull', text: 'Taurus the Bull represents the second astrological sign of the zodiac. In astrology, Taurus is associated with determination, patience, stability, and material security. The constellation represents the bull that carried Europa across the sea in Greek mythology. Taureans are known for their reliable, practical, and persistent nature.' };
    }
    return { title: '🌌 Taurus Constellation', text: 'Taurus is a prominent zodiac constellation containing two of the most famous star clusters: the Pleiades (Seven Sisters) and the Hyades. Aldebaran, the red giant star, forms the Bull\'s fiery eye. You can scroll to zoom and drag to rotate the view!' };
  };

  const cleanupARSession = () => {
    console.log('Taurus AR: Cleaning up AR session...');
    ...
    console.log('Taurus AR: Session cleanup complete');
  };

  const handleBack = () => {
    console.log('Taurus AR: Back button clicked - cleaning up and returning to stars');
    cleanupARSession();
    ...
    <>
      <style>{`
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .back-button { position: absolute; top: 20px; left: 20px; z-index: 1000; display: flex; align-items: center; gap: 8px; padding: 12px 20px; background: rgba(230, 126, 34, 0.2); border: 2px solid #e67e22; border-radius: 12px; color: white; font-weight: 600; cursor: pointer; transition: all 0.3s; }
        .back-button:hover { background: rgba(230, 126, 34, 0.4); }
        .button-group { position: absolute; top: 20px; right: 20px; z-index: 1000; display: flex; gap: 10px; }
        .view-button { padding: 10px 16px; background: rgba(230, 126, 34, 0.2); border: 2px solid #e67e22; border-radius: 12px; color: white; font-weight: 600; cursor: pointer; transition: all 0.3s; font-size: 14px; }
        .view-button:hover { background: rgba(230, 126, 34, 0.4); }
        .view-button.active { background: rgba(230, 126, 34, 0.6); border-color: #fff; }
        .title { position: absolute; top: 80px; left: 50%; transform: translateX(-50%); z-index: 1000; font-size: 24px; font-weight: 700; color: #e67e22; text-shadow: 0 0 10px rgba(230, 126, 34, 0.5); }
        .info-box { 
          position: absolute; 
          bottom: 20px; 
          left: 50%; 
      ...
      {!scriptsLoaded && <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: '#1a1a1a', display: 'flex', justifyContent: 'center', alignItems: 'center', color: 'white' }}><h2>Loading Taurus AR...</h2></div>}
      ...
      <h1 className="title">♉ Taurus AR</h1>
