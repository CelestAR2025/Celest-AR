...
const Virgo = () => {
  ...
  useEffect(() => {
    ...
    const handlePopState = () => {
      console.log('Virgo AR: Browser back button detected - cleaning up AR session');
      cleanupARSession();
  ...
  useEffect(() => {
    ...
    let imageSrc = '/models/2D/virgo.png';
    if (currentView === 'withNames') imageSrc = '/models/2D/virgo-with-name.png';
    else if (currentView === 'withSign') imageSrc = '/models/2D/virgo-with-sign.png';

    console.log('Virgo - Current view:', currentView);
    console.log('Virgo - Image source:', imageSrc);
  ...
  const getInfoContent = () => {
    if (currentView === 'withNames') {
      return { title: '✨ Virgo with Star Names', text: 'This view shows Virgo the Maiden with labeled stars including Spica (Alpha Virginis, a brilliant blue giant and one of the brightest stars in the sky), Porrima (Gamma Virginis, a beautiful double star), Vindemiatrix (the grape gatherer), and Zavijava. Spica is actually a close binary star system!' };
    } else if (currentView === 'withSign') {
      return { title: '♍ Virgo the Maiden', text: 'Virgo the Maiden represents the sixth astrological sign of the zodiac. In astrology, Virgo is associated with purity, wisdom, harvest, service, and attention to detail. The constellation represents Persephone or Astraea, goddesses of justice and purity. Virgos are known for their analytical, practical, and perfectionist nature.' };
    }
    return { title: '🌌 Virgo Constellation', text: 'Virgo is the largest zodiac constellation and second-largest constellation overall. Spica, its brightest star, is a blue giant. The constellation contains the Virgo Cluster, home to over 1,000 galaxies! You can scroll to zoom and drag to rotate the view!' };
  };

  const cleanupARSession = () => {
    console.log('Virgo AR: Cleaning up AR session...');
    ...
    console.log('Virgo AR: Session cleanup complete');
  };

  const handleBack = () => {
    console.log('Virgo AR: Back button clicked - cleaning up and returning to stars');
    cleanupARSession();
    ...
    <>
      <style>{`
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .back-button { position: absolute; top: 20px; left: 20px; z-index: 1000; display: flex; align-items: center; gap: 8px; padding: 12px 20px; background: rgba(46, 204, 113, 0.2); border: 2px solid #2ecc71; border-radius: 12px; color: white; font-weight: 600; cursor: pointer; transition: all 0.3s; }
        .back-button:hover { background: rgba(46, 204, 113, 0.4); }
        .button-group { position: absolute; top: 20px; right: 20px; z-index: 1000; display: flex; gap: 10px; }
        .view-button { padding: 10px 16px; background: rgba(46, 204, 113, 0.2); border: 2px solid #2ecc71; border-radius: 12px; color: white; font-weight: 600; cursor: pointer; transition: all 0.3s; font-size: 14px; }
        .view-button:hover { background: rgba(46, 204, 113, 0.4); }
        .view-button.active { background: rgba(46, 204, 113, 0.6); border-color: #fff; }
        .title { position: absolute; top: 80px; left: 50%; transform: translateX(-50%); z-index: 1000; font-size: 24px; font-weight: 700; color: #2ecc71; text-shadow: 0 0 10px rgba(46, 204, 113, 0.5); }
        .info-box { 
          position: absolute; 
          bottom: 20px; 
          left: 50%; 
      ...
      {!scriptsLoaded && <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: '#1a1a1a', display: 'flex', justifyContent: 'center', alignItems: 'center', color: 'white' }}><h2>Loading Virgo AR...</h2></div>}
      ...
      <h1 className="title">♍ Virgo AR</h1>
